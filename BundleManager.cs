    ...
    internal class BundleManager
    {
        ...
        private void CompletionMessage()
        {
            List<(string,int)> messages = new List<(string, int)> ()
            {
                ...
                ($"Slava Ukraine", 3),
                ($"Well well well, did you really download the source code just to take a sneak peak of all these messages? If you're wondering, the only reason I've scripted them is because I'm bored waiting for the cache to generate.", 0),
